...
- **Ngày thực hiện:** 02/09/2026 (cập nhật lần 11 — **English launch pack**: kiến trúc đa ngôn ngữ vi/en với 27 route `/en/`, bộ chuyển ngôn ngữ, Pagefind lọc theo ngôn ngữ, SEO hreflang/x-default và kiểm tự động `check-english-launch`; tiếng Việt vẫn là ngôn ngữ mặc định, mọi URL tiếng Việt giữ nguyên).
...
- **Kết quả build ngày 02/09/2026 (kết quả build thật, đếm từ `dist/` sau build, gồm English launch pack và timeline Submariner song ngữ):** **249 trang (222 tiếng Việt + 27 tiếng Anh)**, **17.300 liên kết nội bộ hợp lệ**, không có liên kết nội bộ hỏng; sitemap gồm 27 URL `/en/`. Chuỗi `npm run build` nay bao gồm **script chống hồi quy tải 3D (`scripts/check-3d-loading-budget.mjs`)** chạy ngay sau Astro build: xác nhận Three.js chỉ nằm trong các chunk tải khi người dùng chủ động mở tab "Mô hình 3D" trên `/giai-phau/` (engine 131.3 KB gzip + OrbitControls 4.4 KB gzip, cache riêng) và không route nào — kể cả `/giai-phau/` ở sơ đồ 2D mặc định — tải chunk 3D trong luồng ban đầu; và **kiểm route timeline song ngữ (`scripts/check-evolution-routes.mjs`)** cuối chuỗi. Số liệu là build cục bộ, chưa thay thế dữ liệu Core Web Vitals production.
...
- **English launch pack — bản tiếng Anh thật cho người đọc quốc tế** — kiến trúc: `astro.config` locales `['vi','en']` (vi mặc định, không tiền tố; en bắt đầu `/en/`); bảng địa chỉ trung tâm `src/i18n/contentRoutes.ts` (27 cặp vi↔en — nguồn duy nhất cho switcher, hreflang, footer, link trong template); `getSlug` ưu tiên `custom_slug` (không file vi nào có — URL vi nguyên vẹn); 5 template bài dùng chung (`src/components/templates/`) + wrapper mỏng cho route vi và en; **27 route `/en/` xuất bản**: trang chủ, 5 trang tĩnh (about/accessibility/contact/copyright/learning-path), 5 trang danh sách, 16 bài nền tảng (1 guide, 3 cơ chế, 6 thuật ngữ, Rolex/Omega/Seiko, Submariner/Speedmaster/Tank) cùng dữ kiện + nguồn với bản vi, thuật ngữ horology chuẩn. Bộ chuyển ngôn ngữ là link thật ở Header (desktop + mobile): trang chưa dịch → về trang chủ đích kèm aria-label giải thích. Pagefind lọc kết quả theo `language` của trang (tìm "escapement" trên EN trả 8/8 trang `/en/`, trên VI 8/8 trang vi). SEO: `og:locale`/`inLanguage` động, hreflang vi+en+x-default chỉ khi cặp tồn tại, sitemap gồm 27 URL `/en/`, không redirect theo ngôn ngữ. Phần tương tác nhãn tiếng Việt (infographic, DecisionMap, công cụ cầm tay) ẩn ở bản EN thay vì hiện nửa dịch; **ngoại lệ: sơ đồ tiến hóa Submariner đã song ngữ từ Prompt 31** — hiển thị trên cả hai route theo `publishedLangs`. History/Anatomy/Compare đặt ngoài menu English giai đoạn này. Kiểm tự động `scripts/check-english-launch.mjs` (9 nhóm tiêu chí, chạy sau build). **(Hoàn tất 02/09/2026; biên bản `docs/nghiem-thu/2026-09-02_nghiem-thu-english-launch-pack.md`. Giới hạn trung thực: đây là launch pack nền tảng — toàn bộ khối nội dung chưa dịch tiếp tục chỉ tiếng Việt và được giấu khỏi English, không tạo trải nghiệm rỗng; chưa có dữ liệu Search Console.)**
