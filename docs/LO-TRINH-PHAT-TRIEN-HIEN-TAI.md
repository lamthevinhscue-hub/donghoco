...
- **Kết quả build ngày 02/09/2026 (kết quả build thật, đếm từ `dist/` sau build, gồm English launch pack):** **249 trang (222 tiếng Việt + 27 tiếng Anh)**, **17.292 liên kết nội bộ hợp lệ**, không có liên kết nội bộ hỏng; sitemap gồm 27 URL `/en/`. Chuỗi `npm run build` nay bao gồm **script chống hồi quy tải 3D (`scripts/check-3d-loading-budget.mjs`)** chạy ngay sau Astro build: xác nhận Three.js chỉ nằm trong các chunk tải khi người dùng chủ động mở tab "Mô hình 3D" trên `/giai-phau/` (engine 131.3 KB gzip + OrbitControls 4.4 KB gzip, cache riêng) và không route nào — kể cả `/giai-phau/` ở sơ đồ 2D mặc định — tải chunk 3D trong luồng ban đầu. Số liệu là build cục bộ, chưa thay thế dữ liệu Core Web Vitals production.
