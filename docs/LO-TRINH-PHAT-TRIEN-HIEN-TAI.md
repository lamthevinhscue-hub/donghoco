...
- **Ngày thực hiện:** 04/09/2026 (cập nhật lần 15 — **cụm "Automatic & năng lượng cơ học" song ngữ**: chuẩn hóa theo nguồn FHH 6 bài Việt về automatic/rotor/thùng cót/trữ cót/hộp xoay, rà 3 bài English có sẵn rồi xuất bản 3 bài English mới; tiếng Việt vẫn là ngôn ngữ mặc định, mọi URL tiếng Việt giữ nguyên. Lần 14: cụm "Sử dụng an toàn hằng ngày" song ngữ — chuẩn hóa nguồn 6 bài Việt về lên dây/chống nước/chỉnh lịch/núm vặn/dây cót rồi xuất bản 6 bài English mới; lần 13: cụm Chronograph & Tachymeter song ngữ; lần 12: cụm GMT song ngữ).
- **Commit nền của gói:** `9cebd5c` — "feat(i18n): add sourced bilingual daily care knowledge cluster" (trước đó: `5468201` cụm GMT song ngữ, `fe917eb` GMT-Master EN + timeline song ngữ, `477163a` timeline Submariner song ngữ, `7ea7587` English launch pack, `e8a2ff5` cụm bảo dưỡng). Đây là commit nền khi bắt đầu gói; chi tiết thay đổi và kết quả kiểm tra được ghi tại biên bản nghiệm thu cụm automatic & năng lượng cơ học (`docs/nghiem-thu/2026-09-04_nghiem-thu-cum-automatic-nang-luong-song-ngu.md`).
- **Nhánh hiện tại:** `main`.
- **Kết quả `npm run check`:** ĐẠT — toàn bộ kiểm tra nội dung tĩnh, nhóm kiểm WCAG tự động, **kiểm tra liên kết biên tập (`scripts/check-editorial-links.mjs`)**, **kiểm dữ liệu sơ đồ tiến hóa (`scripts/check-evolution-data.mjs`)**, **kiểm dữ liệu lộ trình học (`scripts/check-learning-paths.mjs`)**, **kiểm cụm "chọn đồng hồ cơ đầu tiên" (`scripts/check-first-watch-cluster.mjs`)**, **kiểm cụm "bảo dưỡng & sử dụng an toàn" (`scripts/check-care-cluster.mjs`)**, **kiểm nguồn cụm GMT (`scripts/check-gmt-source-integrity.mjs`)**, **kiểm cụm Chronograph & Tachymeter (`scripts/check-chronograph-cluster.mjs`)** và **kiểm cụm sử dụng an toàn song ngữ (`scripts/check-daily-care-cluster.mjs`)** và **kiểm cụm automatic & năng lượng cơ học song ngữ (`scripts/check-automatic-energy-cluster.mjs`)** không có lỗi. Chạy riêng sau build: **`scripts/check-english-launch.mjs`** (9 nhóm tiêu chí kiến trúc đa ngôn ngữ — ĐẠT).
- **Kết quả build ngày 04/09/2026 (kết quả build thật, đếm từ `dist/` sau build, gồm English launch pack, timeline Submariner + GMT-Master song ngữ, cụm GMT, cụm Chronograph & Tachymeter, cụm "Sử dụng an toàn hằng ngày" và cụm "Automatic & năng lượng cơ học" song ngữ):** **266 trang (222 tiếng Việt + 44 tiếng Anh)**, **18.690 liên kết nội bộ hợp lệ**, không có liên kết nội bộ hỏng; sitemap gồm 265 URL, trong đó **44 URL `/en/`**; **English launch pack đủ 40 route bắt buộc trong `dist`** (tăng từ 37 trước gói). Chuỗi `npm run build` nay bao gồm **script chống hồi quy tải 3D (`scripts/check-3d-loading-budget.mjs`)** chạy ngay sau Astro build: xác nhận Three.js chỉ nằm trong các chunk tải khi người dùng chủ động mở tab "Mô hình 3D" trên `/giai-phau/` (engine 131.3 KB gzip + OrbitControls 4.4 KB gzip, cache riêng) và không route nào — kể cả `/giai-phau/` ở sơ đồ 2D mặc định — tải chunk 3D trong luồng ban đầu; và **kiểm route timeline song ngữ (`scripts/check-evolution-routes.mjs`)** cuối chuỗi. Số liệu là build cục bộ, chưa thay thế dữ liệu Core Web Vitals production.
...
Nguồn tham khảo: 73/73 thương hiệu, 69/69 mẫu iconic, 18/18 cơ chế, 33/33 thuật ngữ đều có khối nguồn; **14/14 bài hướng dẫn có nguồn** (4 bài cụm "bảo dưỡng & sử dụng an toàn" được bổ nguồn ngày 02/09/2026 — trước đó 10/14; hồ sơ tại `docs/ho-so-nguon-bao-duong-va-su-dung-an-toan.md`).
