...
- **Ngày thực hiện:** 03/09/2026 (cập nhật lần 12 — **cụm kiến thức GMT song ngữ**: kiểm chứng và chuẩn hóa dữ kiện GMT theo nguồn, 3 bài tiếng Anh mới cho cụm GMT, autolink từ điển trên trang EN chỉ link thuật ngữ đã có bản EN; tiếng Việt vẫn là ngôn ngữ mặc định, mọi URL tiếng Việt giữ nguyên).
- **Commit nền của gói:** `fe917eb` — "feat(i18n): hoàn thiện cụm Rolex quốc tế — GMT-Master EN + timeline song ngữ" (trước đó: `477163a` timeline Submariner song ngữ, `7ea7587` English launch pack, `e8a2ff5` cụm bảo dưỡng, `ed4181a` cụm chọn đồng hồ đầu tiên). Đây là commit nền khi bắt đầu gói; chi tiết thay đổi và kết quả kiểm tra được ghi tại biên bản nghiệm thu cụm GMT song ngữ.
- **Nhánh hiện tại:** `main`.
- **Kết quả `npm run check`:** ĐẠT — toàn bộ kiểm tra nội dung tĩnh, nhóm kiểm WCAG tự động, **kiểm tra liên kết biên tập (`scripts/check-editorial-links.mjs`)**, **kiểm dữ liệu sơ đồ tiến hóa (`scripts/check-evolution-data.mjs`)**, **kiểm dữ liệu lộ trình học (`scripts/check-learning-paths.mjs`)**, **kiểm cụm "chọn đồng hồ cơ đầu tiên" (`scripts/check-first-watch-cluster.mjs`)** và **kiểm cụm "bảo dưỡng & sử dụng an toàn" (`scripts/check-care-cluster.mjs`)** không có lỗi. Chạy riêng sau build: **`scripts/check-english-launch.mjs`** (9 nhóm tiêu chí kiến trúc đa ngôn ngữ — ĐẠT).
- **Kết quả build ngày 03/09/2026 (kết quả build thật, đếm từ `dist/` sau build, gồm English launch pack, timeline Submariner + GMT-Master song ngữ, cụm GMT song ngữ):** **253 trang (222 tiếng Việt + 31 tiếng Anh)**, **17.609 liên kết nội bộ hợp lệ**, không có liên kết nội bộ hỏng; sitemap gồm 31 URL `/en/`. Chuỗi `npm run build` nay bao gồm **script chống hồi quy tải 3D (`scripts/check-3d-loading-budget.mjs`)** chạy ngay sau Astro build: xác nhận Three.js chỉ nằm trong các chunk tải khi người dùng chủ động mở tab "Mô hình 3D" trên `/giai-phau/` (engine 131.3 KB gzip + OrbitControls 4.4 KB gzip, cache riêng) và không route nào — kể cả `/giai-phau/` ở sơ đồ 2D mặc định — tải chunk 3D trong luồng ban đầu; và **kiểm route timeline song ngữ (`scripts/check-evolution-routes.mjs`)** cuối chuỗi. Số liệu là build cục bộ, chưa thay thế dữ liệu Core Web Vitals production.
...
- **English launch pack — bản tiếng Anh thật cho người đọc quốc tế** — kiến trúc: `astro.config` locales `['vi','en']` (vi mặc định, không tiền tố; en bắt đầu `/en/`); bảng địa chỉ trung tâm `src/i18n/contentRoutes.ts` (31 cặp vi↔en — nguồn duy nhất cho switcher, hreflang, footer, link trong template); `getSlug` ưu tiên `custom_slug` (không file vi nào có — URL vi nguyên vẹn); 5 template bài dùng chung (`src/components/templates/`) + wrapper mỏng cho route vi và en; **31 route `/en/` xuất bản**: trang chủ, 5 trang tĩnh (about/accessibility/contact/copyright/learning-path), 5 trang danh sách, 20 bài nền tảng (2 guide, 4 cơ chế, 7 thuật ngữ, Rolex/Omega/Seiko, Submariner/Speedmaster/Tank/**GMT-Master**) cùng dữ kiện + nguồn với bản vi, thuật ngữ horology chuẩn. **Cụm GMT song ngữ** (cơ chế `gmt`, thuật ngữ `gmt`, hướng dẫn `reading-and-setting-gmt`) xuất bản sau khi kiểm chứng và chuẩn hóa dữ kiện theo nguồn — hồ sơ `docs/ho-so-nguon-cum-gmt-song-ngu.md`; autolink từ điển trên trang EN chỉ bọc thuật ngữ đã có cặp EN (lấy từ bảng contentRoutes), thuật ngữ chưa dịch giữ nguyên văn thay vì link về route tiếng Việt. Bộ chuyển ngôn ngữ là link thật ở Header (desktop + mobile): trang chưa dịch → về trang chủ đích kèm aria-label giải thích. Pagefind lọc kết quả theo `language` của trang (tìm "escapement" trên EN trả 8/8 trang `/en/`, trên VI 8/8 trang vi). SEO: `og:locale`/`inLanguage` động, hreflang vi+en+x-default chỉ khi cặp tồn tại, sitemap gồm 31 URL `/en/`, không redirect theo ngôn ngữ. Phần tương tác nhãn tiếng Việt (infographic, DecisionMap, công cụ cầm tay) ẩn ở bản EN thay vì hiện nửa dịch; **hai timeline tiến hóa đã song ngữ: Rolex Submariner và Rolex GMT-Master** — hiển thị trên cả hai route; timeline khác chỉ hiện English khi dataset của nó có `publishedLangs` chứa `en` (chưa dịch thì ẩn — trạng thái hợp lệ, không phải lỗi). History/Anatomy/Compare đặt ngoài menu English giai đoạn này. Kiểm tự động `scripts/check-english-launch.mjs` (9 nhóm tiêu chí, chạy sau build). **(Hoàn tất 02/09/2026; cụm GMT bổ sung 03/09/2026 — biên bản `docs/nghiem-thu/2026-09-02_nghiem-thu-english-launch-pack.md`, `docs/nghiem-thu/2026-09-03_nghiem-thu-cum-gmt-song-ngu.md`. Giới hạn trung thực: đây là launch pack nền tảng — toàn bộ khối nội dung chưa dịch tiếp tục chỉ tiếng Việt và được giấu khỏi English, không tạo trải nghiệm rỗng; chưa có dữ liệu Search Console.)**
