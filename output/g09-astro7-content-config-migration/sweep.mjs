// G09-C Đ1 chặng di trú Content Collections — sweep định dạng + JSON + nhạy cảm
// (khung sweep-g09-c.mjs: kyLa, nhayCam ghép mảnh chuỗi, whitespace/EOF, JSON;
// log-* miễn kyLa vì npm/astro in ANSI; tệp sửa ít dòng quét nguyên tệp — ký tự
// lạ trùng dãy codePoint với HEAD là hiện trạng có trước)
import { readFileSync, existsSync, readdirSync } from 'node:fs';
import { execFileSync } from 'node:child_process';

const goi = 'output/g09-astro7-content-config-migration';
const files = [
  'docs/nghiem-thu/G09-C-di-tru-content-config-astro7-2026-09-20.md',
  'package.json',
  'postcss.config.mjs',
  'astro.config.mjs',
  'src/content.config.ts',
  'src/pages/ban-quyen.astro',
  'src/components/infographics/MechanismAnimation.astro',
  ...readdirSync(goi).filter((f) => !f.startsWith('log-sweep')).map((f) => goi + '/' + f),
];

const CO_TRUOC = ['src/components/infographics/MechanismAnimation.astro', 'src/pages/ban-quyen.astro'];
