---
title: "Seiko"
custom_slug: "seiko"
excerpt: "The brand that triggered the Quartz Crisis — and never stopped making mechanical watches."
country: "Japan"
tier: "entry-level luxury"
founded: 1881
date: "2026-07-31"
draft: false

sources:
  - label: "Seiko Group Corporation — History"
    url: "https://www.seiko.co.jp/en/group/history/"
  - label: "The Seiko Museum Ginza — Stage 1 (1881–1920s)"
    url: "https://museum.seiko.co.jp/en/seiko_history/company/company/001/"
  - label: "Seiko Watch Corporation — 1965 Diver's Re-creation"
    url: "https://www.seikowatches.com/us-en/products/prospex/special/1965_6L/index"
  - label: "Seiko Design 140 — 1969 Quartz Astron"
    url: "https://by.seiko-design.com/140th/en/topic/32.html"
---

## Overview

Seiko is the brand that **triggered the greatest crisis in mechanical watchmaking**. The Quartz Astron of 1969, the world's first quartz wristwatch, nearly wiped out the Swiss industry; yet Seiko itself never stopped making mechanical watches.

The name "Seikosha", given by Kintaro Hattori to his factory in 1892, means **"the house of exquisite workmanship and precision"** — a statement of intent made before the company had produced a single wristwatch.

## Key milestones

- **1881** — Kintaro Hattori opens a clock repair and sales shop in Tokyo.
- **1892** — The Seikosha factory is founded, producing wall clocks.
- **1913** — The **Laurel**, Japan's first wristwatch — so laborious to make that output reached roughly 50 pieces a day.
- **1965** — Seiko's — and Japan's — **first diving watch**: automatic, water-resistant to 150m, used by the Japanese Antarctic expedition from 1966 to 1969 and known to collectors as the 62MAS.
- **1969** — **Calibre 6139**: one of three automatic chronograph projects to reach the finish line in the same year.
- **1969** — **Quartz Astron**: the world's first quartz wristwatch, accurate to about 0.2 seconds per day — roughly a hundred times better than contemporary mechanical watches. The opening shot of the Quartz Crisis.

## Watchmaking philosophy

- **Vertical independence.** Seiko makes nearly everything in-house — movements, hairsprings, dials.
- **Pragmatism first.** A good watch is durable, accurate and wearable every day — not a safe queen.
- **Comfortable competing with itself.** Seiko 5 and high-end Grand Seiko have coexisted under one roof for decades.

(Grand Seiko now stands as a separate brand and is covered on its own page.)

## Key lines

- **Seiko 5 Sports** — the gateway to mechanical watches for millions.
- **Prospex** — purpose-built dive and sports watches, heirs of the 1965 diver.
- **Presage** — dress watches with handcrafted dials: enamel, urushi lacquer, washi paper.
- **King Seiko** — sharp vintage-flavored design between Seiko and Grand Seiko.

## Related reading

- [Power reserve](/en/mechanisms/power-reserve/) — how long a mechanical watch runs
- [The escapement](/en/mechanisms/escapement/) — the mechanism behind every beat
